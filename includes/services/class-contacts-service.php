...
class Handik_Booking_App_Contacts_Service {
	...
	public function admin_create( array $payload ) {
		...
		$this->logger->info(
			'Admin created contact.',
			array( 'contact_id' => $id, 'admin_id' => get_current_user_id() )
		);
		return $id;
	}

	/**
	 * Update editable fields on a contact. Phone changes are logged at warning level.
	 *
	 * @param int                  $contact_id Contact ID.
	 ...
	 *
	 * @param array<string, mixed> $args Filters: include_spam, with_bookings, drafts_only,
	 *                                   no_address, search, limit, offset.
	 * @return array<int, array<string, mixed>>
	 */
